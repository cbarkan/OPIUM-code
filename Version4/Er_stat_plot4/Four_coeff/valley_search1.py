...
def slope_sign_find():
    plus = err_check(1.0,coeff_vector,slope,'coeff')
    minus = err_check(-1.0,coeff_vector,slope,'coeff')
    if plus < minus:
        return 1
    else:
        return -1
...
def valley_basis_find():
    ...
    for ii in range(R.shape[0]):
        R[ii] = np.zeros(coeff_terms)
        R[ii,0] = 1
        R[ii,ii+1] = -1*slope[0]/slope[ii+1]
        
        coeff_vector = R[ii] + origin_displacement
        slope_approx = slope_sign_find()*slope
        alpha = alpha_find(10.0,'coeff')
        R[ii] = R[ii] - alpha*slope_approx
        
        #Check for length and linear dependece
        for i in range(ii+1):
            dotProd = np.dot(R[ii],R[i])
            if np.absolute(dotProd) < 0.02:
                print ii
                print i
