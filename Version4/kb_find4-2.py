#Version 4-2: slope is a float--it only affects the last coefficient
##################################
...
def err_check(alpha,coeff_vector,slope): #Runs OPIUM, checks test config errors and returns their sum
    ...
    coeff_vector_test[-1] = coeff_vector[-1] - slope*alpha*dC
...
def slope_find(deltaC):
    ...
    if up_slope*down_slope >= 0: #If slopes have the same sign
        ...
        print 'Inconsistent slopes: reducing deltaC'
...
x_min = 0.01
x_max = 1.5
coeff_vector = np.array([11.0,11.0])
new_terms = 8
test_range = 3.0
test_step = 0.25
dC = 0.05
...
for jj in range(new_terms+1):
    ...
    coeff_vector[-1] = coeff_vector[-1] - slope*alpha*dC
