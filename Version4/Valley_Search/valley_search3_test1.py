#Includes Inception Valleys
#Change err_check to ask for coeff space or valley space, and to return error accordingly. This allows the same slope_find and alpha_find functions to be used for search in both coeff space and valley space
...
def err_check(alpha,coord_vector,slope,space_type): #Runs OPIUM, checks test config errors and returns their sum
	...
	if space_type == 'coeff':
		...
		grid = fit_to_func(test_vector,'fourier_cos')
		setKB(grid)
	elif space_type == 'innerValley':
		vVect = np.dot(B,coord_vector - alpha*slope)
		test_vector = VtoC(vVect)
		grid = fit_to_func(test_vector,'fourier_cos')
		setKB(grid)
	else:
...
def alpha_find(alpha_step,space_type):
	...
	if space_type == 'coeff':
		...
		coord_vector = innerValley_vector
	else:
		raise ValueError('space_type must be "coeff" or "valley"')
...
def slope_find(coord_vector,space_type):
	...
	if space_type == 'coeff':
		...
		slope = np.zeros(innerValley_terms)
		terms = innerValley_terms
	else:
		raise ValueError('space_type must be "coeff" or "valley"')
...
def slope_sign_find(coord_vector,space_type):
	plus = err_check(slopeCheckAlpha,coord_vector,slope,space_type)
	minus = err_check(-1*slopeCheckAlpha,coord_vector,slope,space_type)
	if plus < minus:
		return 1
	else:
		return -1
...
def valley_basis_find(lower_space_terms,higher_space_terms,higher_space_type):
	...
	for ii in range(lower_space_terms):
		...
		slope = slope*slope_sign_find(coeff_vector,higher_space_type)
		alpha = alpha_find(10.0,'coeff')
		V[ii] = V[ii] - alpha*slope
		
	M = V.T
	
	print 'coeff <- valley change of basis matrix:'
	print M

	return M

def innerValley_basis_find(lower_space_terms,higher_space_terms,higher_space_type):
	R = np.zeros((lower_space_terms,higher_space_terms))
	for ii in range(lower_space_terms):
		search_disp = 0.01
		R[ii,0] = search_disp
	...
	for ii in range(lower_space_terms):
		coeff_vector = V[ii]
		slope = slope*slope_sign_find(coeff_vector,higher_space_type)
		alpha = alpha_find(10.0,'valley')
		V[ii] = V[ii] - alpha*slope
		
	M = V.T
	print 'valley <- innerValley change of basis matrix:'
	print M
...
coeff_vector = np.array([-4.03337113 ,-7.97575411 ,-3.88896453, -0.12747901])
...
for prim_iter in range(100):
	...
	A = valley_basis_find(valley_terms,coeff_terms,'coeff')
	...
	for sec_iter in range(100):
		...
		alpha = alpha_find(alpha_step,'valley')
		if alpha == 0:
	...
	if best_coeff_iter != prim_iter:
		...
		B = innerValley_basis_find(innerValley_terms,valley_terms,'valley')
		innerValley_vector = np.zeros(innerValley_terms)
		
		print '\nbeginning tertiary iteration\n'
		for thrd_it in range(100):
			print 'Optimizing through innerValley space\n'
			slope = slope_find(innerValley_vector,'innerValley')
			alpha = alpha_find(alpha_step,'innerValley')
			if alpha == 0:
				print '\nReached minimum in tangent-innerValley space. Searching for true innerValley space.\n'
				break
			innerValley_vector = innerValley_vector - alpha*slope
