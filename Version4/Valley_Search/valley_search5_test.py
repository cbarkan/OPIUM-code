#Includes 1 level of Inception Valley find
#New best_errors are found in innerValley space -> it's working!
#Very inefficient, improve conditions for "giving up"
#Maybe try to improve alpha_step choices
...
def err_check(alpha,coord_vector,slope,space_type): #Runs OPIUM, checks test config errors and returns their sum
	...
	if space_type == 'coeff':
		...
		vVect = innerOrigin_displacement + np.dot(B,coord_vector - alpha*slope)
	...
	if rpt[-1] == '#kb_find read-receipt': #Error occured in OPIUM and no new .rpt file was produced
		...
		global best_coeff_iter_error
		if Er_stat < best_coeff_vector_error:
			if best_coeff_vector_error - Er_stat > 0.0000001:
				best_coeff_iter = prim_iter
				best_coeff_iter_error = Er_stat
	...
	if ErU < ErmL:
		...
		for i in range(10000):
			...
			if ErU < ErmL:
				...
				return 0, 0 #If loop completes, no good alpha found, return 0
...
def valley_basis_find(lower_space_terms,higher_space_terms):
	...
	for i in range(higher_space_terms):
		...
		alpha = alpha_find(coeff_vector,10000.0,'coeff')[0]
		R[i] -= alpha*slope
...
def innerValley_basis_find(lower_space_terms,higher_space_terms):
	global slope
	search_disp = 0.05
	R = np.zeros((higher_space_terms,higher_space_terms))
	for i in range(higher_space_terms):
		R[i,i] = search_disp
		valley_vector = R[i] + innerOrigin_displacement
		slope = slope_find(valley_vector,'valley')
		alpha = alpha_find(valley_vector,10000.0,'valley')[0]
		R[i] -= alpha*slope
	norms = [0.0]*higher_space_terms
	for i in range(higher_space_terms):
		norms[i] = np.linalg.norm(R[i])
...
for prim_iter in range(100):
	...
	print '\nFinding valley\n'
	slope = slope_find(coeff_vector,'coeff')
	alpha = alpha_find(coeff_vector,alpha_step,'coeff')[0]
	...
	for sec_iter in range(100):
		...
		slope = slope_find(valley_vector,'valley')
		alpha, Er_imp = alpha_find(valley_vector,alpha_step,'valley')
		if Er_imp < 0.0000001:
			if best_coeff_iter != prim_iter:
				innerOrigin_displacement = valley_vector.copy()
				innerValley_terms = valley_terms - 1
				slope = np.zeros(innerValley_terms)
				B = innerValley_basis_find(innerValley_terms,valley_terms)
				...
				for thrd_it in range(100):
					...
					alpha, Er_imp = alpha_find(innerValley_vector,alpha_step,'innerValley')
					if Er_imp < 0.0000001:
